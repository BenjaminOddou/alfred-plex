        ...
        if "=" in query:
            ...
            for item in split_items:
                ...
                kwargs = {
                    ...
                    if key not in ["advancedFilters", "server", "section", "watchlist"]
                }
    except:
        pass
except IndexError:
...
base_dict = {
    ...
    if key not in ["server", "section", "watchlist"]
}
...
def register_elements(database: list, plex_uuid: str, watchlist: str):
    ...
    if watchlist == 1:
        ...
        filter_database = [
            media for media in database if media.guid not in watchlist_guids
        ]
    ...
    for media in filter_database:
        ...
        if media_type not in ["genre", "photo"] and short_web:
            urlElem = (
                media.getWebURL()
                if media_type not in ["actor", "director"]
                else f"https://app.plex.tv/desktop/#!/provider/tv.plex.provider.discover/details?key=%2Flibrary%2Fpeople%2F{media.tagKey}&context=search"
            )
        ...
        if short_stream and (
            media_type in ["movie", "episode", "clip"]
            or (media_type in ["album", "track"] and media_player != "infuse")
        ):
            sArg = (
                f"_rerun;1;streams;{plex_instance.machineIdentifier}ǀ{media_type}ǀ{media.key}ǀ"
                if media_type not in ["album", "track"] and len(media.media) > 1
                else f"_stream;{plex_instance.machineIdentifier};{media_type};{media.key};0;0"
            )
            if short_stream == "arg":
                media_arg = sArg
            else:
        ...
        if media_type in ["movie", "show"]:
            ...
            if short_mtvsearch != "":
                mtvArg = (
                    f'_mtvsearch;{plexUUID};{media_type};{media.guid.split("/")[-1]}'
                )
        ...
        existing_item = next(
            (item for item in items if item.get("key") == json_obj.get("key")), None
        )
...
if data.get("items"):
    ...
    for obj in data["items"]:
        if (obj["machineIdentifier"] != _machineID and _level > 0 and _machineID) or (
            query_dict.get("server")
            and obj["friendlyName"].lower()
            not in query_dict.get("server", "").lower().split(",")
        ):
            continue
        ...
        try:
            ...
            display_notification(
                "🚨 Error !", f'Failed to connect to the plex server {obj["title"]}'
            )
            ...
            if "=" in query:
                try:
                    ...
                    for section in plex_instance.library.sections():
                        if query_dict.get(
                            "section"
                        ) and section.title.lower() not in query_dict.get(
                            "section", ""
                        ).lower().split(
                            ","
                        ):
                    ...
                    if query == "":
                        ...
                        for section in plex_instance.library.sections():
                            if query_dict.get(
                                ...
                                continue
                            try:
                                database = section.search(
                                    limit=limit_number, **base_dict
                                )
                            except:
                                continue
                    elif not "/" in query:
                        ...
                        database = plex_instance.search(
                            query.split("/")[0],
                            mediatype=query.split("/")[1],
                            limit=limit_number,
                        )
                    register_elements(database, plexUUID, watchlist)
                except:
