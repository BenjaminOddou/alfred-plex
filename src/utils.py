...
cache_folder = os.path.expanduser(
    "~/Library/Caches/com.runningwithcrayons.Alfred/Workflow Data/com.benjamino.plex"
)
...
default_list = [
    ...
    {"title": "default_view", "func": eval},
    {"title": "limit_number", "func": int},
    {"title": "days_history", "func": int},
    {
        "title": "log_level",
    },
    {
...
accounts_file_path = os.path.join(
    data_folder, "accounts.json"
)  # default = ~/Library/Application Support/Alfred/Workflow Data/com.benjamino.plex/servers.jso
servers_file_path = os.path.join(
    data_folder, "servers.json"
)  # default = ~/Library/Application Support/Alfred/Workflow Data/com.benjamino.plex/servers.json
alias_file_path = os.path.join(
    data_folder, "alias.json"
)  # default = ~/Library/Application Support/Alfred/Workflow Data/com.benjamino.plex/alias.json
presets_file_path = os.path.join(
    data_folder, "presets.json"
)  # default = ~/Library/Application Support/Alfred/Workflow Data/com.benjamino.plex/presets.json
config_file_path = os.path.join(
    data_folder, "config.ini"
)  # default = ~/Library/Application Support/Alfred/Workflow Data/com.benjamino.plex/config.ini
...
def presets_file():
    try:
        if (
            not os.path.isfile(presets_file_path)
            or os.path.getsize(presets_file_path) == 0
        ):
...
def default_element(
    _action: str, items: list, query: str = None, query_dict: dict = None
):
    if _action == "no_PMS":
        items.append(
            {
...
def get_plex_account(
    uuid=None, auth_token=None, username=None, password=None, otp=None
):
    plex_account = None
        ...
        try:
            if auth_token:
                ...
                plex_account = MyPlexAccount(
                    username=username, password=password, code=otp
                )
        except Exception as e:
