...
if _action in ["delete", "reset"]:
    if _res == "yes":
        if os.path.exists(_pos):
            ...
            display_notification(
                "✅ Sucess !", f"{_categ.capitalize()} have been removed"
            )
            print(f'1;{_categ.split(" ")[0]};0', end="")
        else:
            display_notification(
                "🚨 Error !",
                f"{_categ.capitalize()} doesn't exists or can't be removed",
            )
    ...
    for item in data:
        if item["short_key"] == _query:
            display_notification(
                "⚠️ Warning !",
                f'The alias {_query} is already used for {item["long_key"]}',
            )
