...
def searchDiscover(query=None, libtype=None, plex_uuid=None):
    ...
    if query:
        ...
        response = requests.get(
            f"{plex_account.DISCOVER}/library/search", params=params1, headers=headers
        )
    else:
        params2 = {
        ...
        response = requests.get(
            f"{plex_account.DISCOVER}/hubs/sections/home/top_watchlisted",
            params=params2,
            headers=headers,
        )
    ...
    if response.status_code == 200:
        ...
        if query:
            ...
            searchResult = [
                ...
                if s.get("id") in ["external", "people"]
                for r in s.get("SearchResult", [])
            ]
        else:
            searchResult = json_data["MediaContainer"].get("Metadata", [])
        for result in searchResult:
            metadata = (
                result.get("Metadata") or result.get("Directory") if query else result
            )
...
if plex_account.get("items"):
    ...
    try:
        database = (
            searchDiscover(query=query, plex_uuid=plex_uuid)
            if not "/" in query
            else searchDiscover(
                query=query.split("/")[0],
                libtype=query.split("/")[1],
                plex_uuid=plex_uuid,
            )
        )
