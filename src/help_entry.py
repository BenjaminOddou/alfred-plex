...
if level == 0:
    ...
    _type, _sectionID = os.environ["_lib"].split(";")[1], int(
        os.environ["_lib"].split(";")[2]
    )
        ...
        if data.get("items"):
            for obj in data["items"]:
                ...
                try:
                    ...
                    display_notification(
                        "🚨 Error !",
                        f'Failed to connect to the plex server {obj.get("title")}',
                    )
    ...
    if level == 1:
            ...
            for o in [
                {
                    ...
                    "subtitle": (
                        transform_path
                        if valid
                        else f"{_type.capitalize()} folder doesn't exists"
                    ),
            ...
            items.append(
                {
                    ...
                    "icon": {"path": "icons/base/refresh.webp"},
                }
            )
            for index, item in enumerate(
                alias_file(_testkey=None, _type=None, _file=True)
            ):
                modified_item = {
                    "title": item["long_key"],
                    "subtitle": f'Alias : {item["short_key"]}',
                    "icon": {"path": "icons/base/alias.webp"},
                    "valid": False,
                    "mods": {
                        "cmd": {
                            "subtitle": "Press ⏎ to modify the alias",
                            "arg": f'_run;modify;{item["long_key"]};{item["short_key"]}',
                            "icon": {"path": "icons/base/modify.webp"},
                        }
                    },
                ...
                items.append(
                    {
                        ...
                        "icon": {"path": f'icons/web/{i.get("img")}.webp'},
                    }
                )
    else:
        if _type == "filter&sort":
        ...
        library_advancedFilter = alias_file(
            _testkey="advancedFilters", _type="alias_or_long"
        )
        sort_filter = alias_file(_testkey="sort", _type="alias_or_long")
        if level == 2:
            if _type == "filter&sort":
                for t in library_subtypes:
                    for f in plex_instance.library.sectionByID(_sectionID).listFilters(
                        libtype=t
                    ):
                    ...
                    for f in plex_instance.library.sectionByID(_sectionID).listFields(
                        libtype=t
                    ):
                        if not "," in f.key:
                    ...
                    for so in plex_instance.library.sectionByID(_sectionID).listSorts(
                        libtype=t
                    ):
                        if not "," in so.key:
            ...
            if _type == "filter&sort":
                ...
                if Type == "filter":
                    if fType == "boolean":
                        choices = json.loads(
                            '[{"key": "1", "title": "is true"}, {"key": "0", "title": "is false"}]',
                            object_hook=lambda d: SimpleNamespace(**d),
                        )
                    else:
                        choices = plex_instance.library.sectionByID(
                            _sectionID
                        ).listFilterChoices(Name)
                    for p in choices:
                        dKey = urllib.parse.unquote(p.key.replace("%25", "%"))
                        pArg = f"{library_filter}={library_type}"
                        pArg += (
                            f"/{Alias}={dKey}" if filters_bool else f"/{Name}={dKey}"
                        )
                        items.append(
                            {
                                "title": p.title,
                    ...
                    for obj in choices:
                        Value = (
                            1
                            if obj.title == "is true"
                            else 0 if obj.title == "is false" else ""
                        )
