            ...
            items.append(
                {
                    ...
                    "icon": {"path": "icons/base/server.webp"},
                }
            )
    else:
            ...
            if level == 1:
                if not owner:
                    ...
                    if not plex_instance.isLatest():
                        ...
                        sSubtitle += (
                            f" -> New version: {plex_instance.checkForUpdate().version}"
                        )
                            ...
                            items.append(
                                {
                                    ...
                                    "icon": {"path": "icons/base/person.webp"},
                                }
                            )
                            break
                    for option in [
                ...
                if _type == "watch":
                    for session in plex_instance.sessions():
                        if session:
                            ...
                            sName = (
                                f"{session.title} ({session.year})"
                                if session.type == "movie"
                                else (
                                    f"{session.grandparentTitle} ǀ {session.title} ǀ {session.seasonEpisode}"
                    ...
                    final_json["rerun"] = 1
                    current_time = datetime.now().replace(microsecond=0) - timedelta(
                        seconds=2
                    )
                    filters = {
                        "at>": current_time,
                    ...
                    bandwidthData = plex_instance.bandwidth(
                        timespan="seconds", **filters
                    )
                ...
                if _type == "setting":
                    for setting in plex_instance.settings.group(_key):
                        if setting.hidden == False:
                            sSumm = (
                                f"ǀ {setting.summary}" if setting.summary != "" else ""
                            )
                            items.append(
                                {
                                    "title": setting.label,
                    ...
                    for h in plex_instance.history(mindate=history_days()):
                        try:
                            lName = plex_instance.library.sectionByID(
                                h.librarySectionID
                            ).title
                        except:
                    ...
                    if lType in ("show", "artist"):
                        ...
                        lSize += ", " + ", ".join(
                            [
                                f"{sectionBase.totalViewSize(libtype=subtype, includeCollections=False)} {subtype}(s)"
                                for subtype in subtypes[lType]
                            ]
                        )
                    for elem in [
                        {
                ...
                if _type == "history":
                    for h in plex_instance.library.sectionByID(int(_key)).history(
                        mindate=history_days()
                    ):
                        lName = plex_instance.library.sectionByID(
                            h.librarySectionID
                        ).title
