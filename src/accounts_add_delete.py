...
if _type == "_delete":
    for item in data["items"]:
        if item.get("uuid") == _input:
            ...
            display_notification(
                "✅ Sucess !", f'The account {item.get("title")} is removed'
            )
            break
elif _type == "_new":
    if _origin == "classic":
        ...
        username, password, otp = (
            lst_idx(credentials, 0),
            lst_idx(credentials, 1),
            lst_idx(credentials, 2),
        )
    ...
    for idx, obj in enumerate(data["items"]):
        if obj.get("uuid") == plex_account.uuid:
            ...
            message = (
                f"{plex_account.friendlyName} plex account informations were updated"
            )
