...
if data.get("items"):
    ...
    try:
        ...
        display_notification(
            "🚨 Error !", f'Failed to connect to the plex server {obj.get("title")}'
        )
        custom_logger("error", e)
        exit()
    try:
        if _type == "version":
            ...
            custom_logger(
                "info",
                f"A new plex version was downloaded at {plex_instance.checkForUpdate().downloadURL}",
            )
        elif _type == "terminateSession":
            plex_instance.query(
                f"/status/sessions/terminate?sessionId={_key}&reason={_msg}"
            )
            display_notification(
                "✅ Sucess !", "The session terminated correctly with your message"
            )
            custom_logger(
                "info", f"The session {_key} terminated with the message {_msg}"
            )
        elif _type == "logs&databases":
            display_notification(
                "⏳ 1/2 Please wait !", f"Downloading {serverName} logs..."
            )
            plex_instance.downloadLogs(savepath=downloads_folder)
            display_notification(
                "⏳ 2/2 Please wait !", f"Downloading {serverName} database..."
            )
            ...
            if _key != "all":
                ...
                message = (
                    f"{_type.capitalize()} started for {libraryName} on {serverName}"
                )
                display_notification("✅ Sucess !", message)
                custom_logger("info", message)
            else:
                ...
                message = (
                    f"{_type.capitalize()} started for all librairies on {serverName}"
                )
                display_notification("✅ Sucess !", message)
                custom_logger("info", message)
    except Exception as e:
